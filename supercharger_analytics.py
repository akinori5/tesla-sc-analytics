...
// Build country code → name mapping from data, and auto-assign colors
const _ccToName = {}; RAW.forEach(r => { if (r.country_code) _ccToName[r.country_code] = getCountry(r.location); });
const _palette = ["#3b82f6","#e31937","#8b5cf6","#22c55e","#f59e0b","#06b6d4","#f472b6","#a78bfa","#fb923c","#4ade80","#e879f9","#38bdf8"];
const CC = {}; Object.values(_ccToName).forEach((name, i) => { CC[name] = _palette[i % _palette.length]; });
const _nameToCode = {}; Object.entries(_ccToName).forEach(([code, name]) => { _nameToCode[name] = code; });
function CF(name) { return countryFlag(_nameToCode[name] || ""); }
...
<footer style="text-align:center;padding:32px 0 24px;color:#555;font-size:12px;font-family:'DM Sans',sans-serif;">
  Generated by <a href="https://github.com/akinori5/tesla-sc-analytics" style="color:#888;text-decoration:underline;" target="_blank">Tesla Supercharger Analytics</a>
</footer>
</body>
</html>"""
